...
SELECT * FROM (
    ...
    SELECT 
        ...
        CASE WHEN cp.procdate <= '2025-02-18' AND (cp.status = 0 OR (cp.status = 1 AND cp.datecp > '2025-02-18')) THEN cp.writeoff ELSE 0 END -- Manually enter when not current date
        ) InsWoEst, 
        ( -- this will equate to 0 if the claim has been received, otherwise it's the insurance pay estimate
        CASE WHEN cp.procdate <= '2025-02-18' AND (cp.status = 0 OR (cp.status = 1 AND cp.datecp > '2025-02-18')) THEN cp.inspayest ELSE 0 END -- Manually enter when not current date
        ) InsPayEst, 
        0 AgedProcNum, 
        '0001-01-01' AgedProcDate 
    FROM 
    ...
    FROM 
        paysplit ps 
    WHERE 
        ps.splitamt != 0 AND ps.datepay <= '2025-02-18' -- Manually enter when not current date
    UNION ALL 
    ...
    WHERE 
        ppc.chargedate <= '2025-02-28' -- 10 days out from the current date
