-- The main goal of this table is to group together transactions that happened on the same day by TranType (negative and positive charges of the same TranType are separated out). It also creates a new TranType, SumByProcAndDate, that just contains adjustments attached to procedures
-- TranType | PriKey | patnum | TranDate | TranAmount | PayPlanAmount | InsWoEst | InsPayEst
SELECT * FROM (
    SELECT 
        ( -- Adjustments are the only transaction type that use agedprocnum
        CASE 
        ...
        0 PriKey, 
        tranbyproc.patnum, 
        ( -- Adjustments are the only transaction type that use agedprocdate
        CASE 
    ...
    GROUP BY 
        ...
        tranbyproc.trandate, 
        ( -- This CASE statement matches the SELECT statement above and allows us to group by the new TranType
        CASE 
            WHEN tranbyproc.agedprocnum = 0 THEN tranbyproc.trantype 
            ELSE 'SumByProcAndDate'
        END
        ), 
        ( -- This CASE statement is used to silently separate charges being grouped together by charges and credits
        CASE 
